function Get-O365AzureElevatedDenyAssignments {
    ...
    if (-not $Headers.HeadersARM) {
        Write-Warning 'Get-O365AzureElevatedDenyAssignments - Azure management token not available. Ensure Connect-O365Admin has permission to access https://management.azure.com.'
        return
    }
    ...
    $items | Where-Object { $_.properties.scope -eq '/' } | ForEach-Object {
        [pscustomobject]@{
            Id            = $_.id
