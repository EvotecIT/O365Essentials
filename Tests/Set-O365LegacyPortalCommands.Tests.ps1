...
Describe 'Legacy MFA settings setter' {
    It 'uses the discovered license key and warns when the service does not persist the value' {
        $script:mfaBody = $null
        Mock -ModuleName O365Essentials Get-O365AzureMultiFactorAuthentication -MockWith {
            [pscustomobject]@{
                tenantId                      = 'tenant-id'
                licenseKey                    = 'ABCDABCDABCD'
                customerId                    = -1
                accountLockoutDurationMinutes = 12
                accountLockoutResetMinutes    = 11
                accountLockoutThreshold       = 6
                allowPhoneMenu                = $false
                blockForFraud                 = $false
                callerId                      = '8553308653'
                defaultBypassTimespan         = 300
                enableFraudAlert              = $true
                fraudCode                     = '0'
                smsTimeoutSeconds             = 59
                caches                        = @()
        ...
        Mock -ModuleName O365Essentials Invoke-O365Admin -MockWith { $script:mfaBody = $Body }
        Mock -ModuleName O365Essentials Write-Warning

        Set-O365AzureMultiFactorAuthentication -Headers @{ HeadersAzure = @{ Authorization = 'Bearer azure' } } -AccountLockoutDurationMinutes 13

        $script:mfaBody.accountLockoutDurationMinutes | Should -Be 13
        Assert-MockCalled Invoke-O365Admin -ModuleName O365Essentials -ParameterFilter {
            $Uri -eq 'https://main.iam.ad.ext.azure.com/api/MultiFactorAuthentication/TenantModel?licenseKey=ABCDABCDABCD' -and
            $Method -eq 'PATCH'
        } -Exactly 1
        Assert-MockCalled Write-Warning -ModuleName O365Essentials -ParameterFilter {
            $Message -like "Set-O365AzureMultiFactorAuthentication - The service did not persist 'accountLockoutDurationMinutes'*"
        } -Exactly 1
    }
}
