...
Describe 'Get-O365PortalContextHeaders' {
    ...
    It 'builds CopilotConnectors context headers using the live portal shape' {
        $Headers = Get-O365PortalContextHeaders -Context CopilotConnectors -PortalHost 'https://admin.cloud.microsoft'
        $Headers['Referer'] | Should -Be 'https://admin.cloud.microsoft/'
        $Headers['x-adminapp-request'] | Should -Be '/copilot/connectors'
